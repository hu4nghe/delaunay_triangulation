...
namespace
{
    ...
    struct face_key_hash
    {
        auto operator()(const face_key& key) const noexcept -> std::size_t
        {
            ...
            return hash_point(key.a) ^ (hash_point(key.b) << 1) ^ (hash_point(key.c) << 2);
        }
    };

    auto canonical_face(const tools_3D::point& p0, const tools_3D::point& p1, const tools_3D::point& p2) -> face_key
    {
...
namespace tools_3D
{
    ...
    {
        std::unordered_map<face_key, std::pair<int, std::array<tools_3D::point, 3>>, face_key_hash> face_usage;
        ...
        for (const auto& tetrahedron : tetrahedra)
        {
            ...
            for (const auto& face : faces)
            {
                ...
                {
                    face_usage.emplace(key, std::make_pair(1, std::array<tools_3D::point, 3>{p0, p1, p2}));
                }
                else
                {
                    it->second.first += 1;
        ...
        auto get_or_add_index = [&](const tools_3D::point& point) -> std::uint32_t
        {
            auto found = vertex_to_index.find(point);
            if (found != vertex_to_index.end())
                return found->second;
