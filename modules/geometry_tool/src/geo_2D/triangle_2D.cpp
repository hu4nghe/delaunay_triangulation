...
namespace tools_2D
{
        ...
        const tools_2D::point& c)
    {
        ...
        if (cross > 0) vertices = {a, b, c};
        else if (cross < 0) vertices = {a, c, b};
        else throw std::invalid_argument("Triangle vertices are collinear");
    }

    tools_2D::triangle::triangle(
        const tools_2D::point&   p,
        const tools_2D::segment& e)
        : vertices{
              p,
              e.get_points().first,
              e.get_points().second}
    ...
    bool tools_2D::triangle::containsVertex(const tools_2D::point& p) const
    {
        return std::ranges::find(vertices, p) != vertices.end();
    }

    tools_2D::circle tools_2D::triangle::circum_circle() const
    {
        ...
        tools_2D::point center(
            vertices[0].get_coordinates().first +
                (ab_len2 * ac.get_coordinates().second -
                 ac_len2 * ab.get_coordinates().second) /
                    d,
            vertices[0].get_coordinates().second +
                (ac_len2 * ab.get_coordinates().first -
                 ab_len2 * ac.get_coordinates().first) /
                    d);

        return tools_2D::circle(
            center,
            std::sqrt(tools_2D::vector_2D(center, vertices[0]).length_sq()));
    }
