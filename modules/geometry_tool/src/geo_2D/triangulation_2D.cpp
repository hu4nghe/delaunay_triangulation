...
namespace
{
    class guibas_stolfi_triangulator
    {
        ...
        auto left_of(const tools_2D::point& p, tools_2D::detail::quad_edge_mesh::edge* e) const -> bool
        {
            return tools_2D::orient2d(points_[e->origin], points_[e->dest()], p) > tools_2D::geometric_epsilon;
        }

        auto right_of(const tools_2D::point& p, tools_2D::detail::quad_edge_mesh::edge* e) const -> bool
        {
            return tools_2D::orient2d(points_[e->origin], points_[e->dest()], p) < -tools_2D::geometric_epsilon;
        }

        auto valid(tools_2D::detail::quad_edge_mesh::edge* e, tools_2D::detail::quad_edge_mesh::edge* basel) const -> bool
        {
            return right_of(points_[e->dest()], basel);
        }

        auto build(std::size_t l, std::size_t r)
            -> std::pair<tools_2D::detail::quad_edge_mesh::edge*, tools_2D::detail::quad_edge_mesh::edge*>
        {
            using edge_ptr = tools_2D::detail::quad_edge_mesh::edge*;

            if (r - l == 1)
            {
                edge_ptr a = mesh_.make_edge(static_cast<int>(l), static_cast<int>(r));
                return {a, a->sym()};
            }

            if (r - l == 2)
            {
                edge_ptr a = mesh_.make_edge(static_cast<int>(l), static_cast<int>(l + 1));
                edge_ptr b = mesh_.make_edge(static_cast<int>(l + 1), static_cast<int>(r));
                mesh_.splice(a->sym(), b);

                const double o = tools_2D::orient2d(points_[l], points_[l + 1], points_[r]);
            ...
            while (true)
            {
                if (left_of(points_[rdi->origin], ldi))
                    ldi = ldi->l_next();
        ...
        auto collect_triangles() const -> std::vector<std::array<int, 3>>
        {
            ...
            for (auto* base : mesh_.primal_edges())
            {
                ...
                {
                    if (!e->alive || e->origin < 0 || e->dest() < 0)
                        continue;
                    ...
                    if (!e1->alive || !e2->alive)
                        continue;
                    if (e1->origin < 0 || e1->dest() < 0 || e2->origin < 0 || e2->dest() < 0)
                        continue;
                    if (e2->l_next() != e)
                        continue;
                    ...
                    if (tools_2D::orient2d(points_[a], points_[b], points_[c]) <= tools_2D::geometric_epsilon)
                        continue;
    ...
    public:
        ...
        auto triangulate() -> std::vector<std::array<int, 3>>
        {
            if (points_.size() < 3)
                return {};
