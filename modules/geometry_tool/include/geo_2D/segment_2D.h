...
namespace tools_2D
{
    class segment
    {
         ...
         * We cannot define the neutral element of segement, there for default
         ...
         */
        auto get_points() const -> std::pair<
            tools_2D::point,
            tools_2D::point>;
    };
} // namespace tools_2D
...
namespace std
{
    template <> struct hash<tools_2D::segment>
    {
        std::size_t operator()(tools_2D::segment const& seg) const noexcept
        {
            auto [a, b] = seg.get_points();
            auto ha     = std::hash<double>{}(a.get_x()) ^
                      (std::hash<double>{}(a.get_y()) << 1);
            auto hb = std::hash<double>{}(b.get_x()) ^
                      (std::hash<double>{}(b.get_y()) << 1);
